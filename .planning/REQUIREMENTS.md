...
- [x] **SERP-02**: Each result link's `href` is the original Brave-provided URL verbatim — never translated, never mutated, never proxied.
- [x] **SERP-03**: Result snippet/title text is rendered XSS-safely — Brave `description` HTML (e.g. `<strong>` highlights) is stripped to plain text; content is never assigned to `innerHTML`.
...
| SERP-02 | Phase 9 | Complete |
| SERP-03 | Phase 9 | Complete |
